...
use ark_poly::{
    DenseUVPolynomial, EvaluationDomain, Evaluations, Polynomial as _, Radix2EvaluationDomain,
};
...
use rayon::iter::{IndexedParallelIterator, IntoParallelIterator as _};
    ...
    E: Pairing,
{
    pub fn setup<R: RngCore>(max_degree: usize, rng: &mut R) -> Result<UniversalParams<E>, Error> {
        ...
        let precommit_time = start_timer!(|| "Precommitting to lagrange polynomials");
    ...
    pub fn compute_opening_proof(
        ...
        let numerator: DensePolynomial<E::ScalarField> = polynomial.clone().sub(&eval_as_poly);
